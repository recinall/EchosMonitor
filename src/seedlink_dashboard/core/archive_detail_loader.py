...
proven-correct :class:`~seedlink_dashboard.core.hvsr_engine.HvsrEngine`
pattern exactly:
...
thread-safe :class:`~seedlink_dashboard.storage.dao.ArchiveDao` (per-thread
sqlite via ``threading.local``), both consulted **read-only** (rule 8). A
fresh :class:`~seedlink_dashboard.storage.archive_reader.ArchiveReader` is
