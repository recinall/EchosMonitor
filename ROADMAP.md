...
| Date | Decision | Why |
|------|----------|-----|
| 2026-07-01 | **Reconnect-replay dedup is CONTENT-VERIFIED, not magnitude-only.** On a SeedLink reconnect the Echos device re-streams its ring buffer — data already written — which obspy delivers as a backward-timestamped `Trace`; the append-only writer (rule 8) cannot dedup, so it persisted these as multi-second SDS overlaps (the recurring 00:23/12:23 UTC events + any post-reconnect replay). `StreamingEngine._on_packet` now keeps a per-stream `_replay_watermark` (last forwarded sample end-time, persists across reconnects, cleared on device stop). A `>= _REPLAY_MIN_OVERLAP_S` (3 s) backward step is dropped (full) / trimmed (partial) BEFORE ring/DSP/gap-detector/writer — but ONLY once `_confirm_replay` matches its overlapping head against the ring-buffer samples for the same span (`_windows_match`: normalized xcorr ≥ 0.95 over a ±3-sample lag search). An UNCONFIRMED big backward step is a genuine device clock reset carrying NEW data → kept, deferred to the GapDetector's clock-jump handling, and the watermark adopts the new timeline. | Empirically (Notte7, Monitor1, Temporale, Notte6) every ≥3 s backward overlap is a byte-identical re-send (xcorr 0.9995–1.0000); overlaps <2 s are a different mid-stream catch-up artefact (scattered xcorr) and are left alone. But magnitude alone can't tell a replay from a real clock reset — both occupy the 8–12 s band — so a content check is the only way to drop redundant data WITHOUT risking silent science-path loss (code-review blocker). The 3 s gate sits in the near-empty 2–3 s band, above device stamp jitter (±2 samples), so the M6.5-B rectification path is untouched. Reviewed by qt-concurrency-auditor (cleanup relocated to `_stop_device` — the watermark is populated unconditionally, so monitoring-only stops must clear it) + code-reviewer (PASS). See docs/POSTMORTEMS.md (2026-07-01). |
| 2026-06-19 | **v0.1.4: fixed HVSR dead-on-Windows (field-verified on Win10 22H2 via SSH).** v0.1.3's off-process HVSR child crashed on every compute on real Windows (`cannot create weak reference to 'NoneType' object`) and `--check` failed — but CI passed, so the broken bundle shipped. Root cause: a windowed (`console=False`) PyInstaller **spawn** child has `sys.stdout is None`, and the child's `structlog.configure` left the default `PrintLoggerFactory` (writes to `sys.stdout`), so the first compute log line `weakref(None)`'d before numba even ran. Fix in `core/hvsr_compute._compute_server_main`: redirect `None` std streams to `os.devnull` AND pin the child's structlog `logger_factory` to that devnull file. Belt-and-braces: `SubprocessHvsrComputeClient` falls back to in-process compute (latching `subprocess_broken`) if the child can't run at all, forwards the child's full traceback over the pipe, and `--check` logs a loud warning (not a failure) when it falls back. | CI runners attach a **console** so `sys.stdout` is a real stream — the bug was invisible there and only appears on a field GUI launch (no console). The off-process boundary is the M6.6 GIL fix (rule 1); restoring it on Windows beats the alternative of leaving HVSR dead. The fallback keeps the feature alive on any *other* future child break, and forwarding the full child traceback (not `str(exc)`) is what turned an opaque `weakref(None)` into a one-look diagnosis — a separate process's pipe is the only window into its failure. See docs/POSTMORTEMS.md (2026-06-19). |
...
| 2026-06-18 | HVSR compute moved **out-of-process** (`core/hvsr_compute.py`): both `HvsrEngine` and `HvsrArrayEngine` run `accumulator.compute()` in a persistent **spawn** child via `SubprocessHvsrComputeClient` (lazy spawn, warm across recomputes/measurements, respawn-on-death, bounded `close`); the worker thread only blocks on a pipe `poll` (GIL released). `InProcessHvsrComputeClient` reproduces the old same-thread behaviour and is the test-suite default via a conftest autouse override of `make_default_compute_client`; `__main__` adds `freeze_support()` + a `--check` subprocess round-trip. | The follow-up the 2026-06-18 stall-watchdog postmortem named: hvsrpy/numba holds the GIL for seconds, so on a QThread it still froze the GUI/engine + SeedLink worker (rule 1/10/11). A subprocess is the only real fix — and a bonus: the compute is now genuinely interruptible (cancel `terminate()`s the child) where in-process numba could only be abandoned mid-JIT. **spawn** not fork (a multi-QThread Qt parent would inherit held locks + the SeedLink socket fds); per-worker child ownership + a client lock serialise the GUI-thread close vs the worker-thread cancel; `--check` gates the frozen-bundle spawn path (same CI-catches-it discipline as the obspy-metadata smoke). Reviewed by code-reviewer + qt-concurrency-auditor (both PASS; low-severity orphan-on-stuck-shutdown noted, daemon child is OS-reaped). |
