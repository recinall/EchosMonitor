"""Off-thread archive detail loader — correctness + the rule-11 invariant.

The loader reads a scrolled-out detection's Z/N/E components from the SDS
archive on a dedicated worker thread (mirroring ``HvsrEngine``) and hands
the GUI thread prepared, NaN-gapped arrays. Two properties matter:

* **Correctness**: it reads the right window for each present component,
  preserves gaps as NaN (never interpolated), returns present-only
  components, emits ``empty`` when nothing is archived, ``failed`` (without
  crashing the thread) on a bad read, and is latest-wins / interruptible.
* **The rule-11 invariant** (the one the reverted Archive Replay got
  wrong): a slow archive read MUST NOT stall the live drain. The test
  asserts the engine's live drain KEEPS ADVANCING (``_latest_raw_endtime``)
  while a deliberately-slow load runs — *not* merely that buffers are
  unshared.
"""

from __future__ import annotations

import time
from pathlib import Path

import numpy as np
from obspy import Stream, Trace, UTCDateTime
from PySide6.QtCore import QObject, Qt, QThread, Signal, Slot

from echosmonitor.config.schema import (
    AppConfig,
    DeviceConfig,
    HighpassStage,
    ReconnectConfig,
    RootConfig,
    StreamSelectorConfig,
    UiConfig,
)
from echosmonitor.core.archive_detail_loader import (
    ArchiveDetailLoader,
    ArchiveDetailResult,
)
from echosmonitor.core.models import StreamID
from echosmonitor.core.streaming_engine import StreamingEngine
from echosmonitor.storage import archive_reader as archive_reader_mod
from echosmonitor.storage.sds import device_sds_root, sds_path

_NET, _STA, _LOC = "IU", "ANMO", "00"
_DEVICE = "anmo"
_FS = 100.0
_PREFIX = "BH"


def _nslc(comp: str) -> str:
    return f"{_NET}.{_STA}.{_LOC}.{_PREFIX}{comp}"


def _components() -> dict[str, str]:
    return {c: _nslc(c) for c in ("Z", "N", "E")}


def _write_trace(root: Path, comp: str, t0: UTCDateTime, npts: int, device: str = _DEVICE) -> None:
    """Write one contiguous component trace into the SDS layout."""
    sid = StreamID(_NET, _STA, _LOC, f"{_PREFIX}{comp}")
    path = sds_path(device_sds_root(root, device), t0, sid)
    path.parent.mkdir(parents=True, exist_ok=True)
    rng = np.random.default_rng(abs(hash(comp)) % (2**32))
    tr = Trace(
        data=(rng.standard_normal(npts) * 1000.0).astype(np.int32),
        header={
            "network": _NET,
            "station": _STA,
            "location": _LOC,
            "channel": f"{_PREFIX}{comp}",
            "starttime": t0,
            "sampling_rate": _FS,
        },
    )
    tr.write(str(path), format="MSEED")


def _write_gapped_trace(root: Path, comp: str, t0: UTCDateTime, npts: int, gap_at: int) -> None:
    """Write a component with a real gap (two sub-traces, missing middle)."""
    sid = StreamID(_NET, _STA, _LOC, f"{_PREFIX}{comp}")
    path = sds_path(device_sds_root(root, _DEVICE), t0, sid)
    path.parent.mkdir(parents=True, exist_ok=True)
    rng = np.random.default_rng(7)
    hdr = {
        "network": _NET,
        "station": _STA,
        "location": _LOC,
        "channel": f"{_PREFIX}{comp}",
        "sampling_rate": _FS,
    }
    first = Trace(
        data=(rng.standard_normal(gap_at) * 1000.0).astype(np.int32),
        header={**hdr, "starttime": t0},
    )
    # Skip ``gap_at`` samples: second sub-trace starts after a real gap.
    second_start = t0 + (gap_at + gap_at) / _FS
    second = Trace(
        data=(rng.standard_normal(npts - gap_at) * 1000.0).astype(np.int32),
        header={**hdr, "starttime": second_start},
    )
    Stream(traces=[first, second]).write(str(path), format="MSEED")


class _Collector(QObject):
    """Captures the loader's GUI-thread signals for assertions."""

    def __init__(self) -> None:
        super().__init__()
        self.loaded: list[ArchiveDetailResult] = []
        self.empty: list[int] = []
        self.failed: list[tuple[int, str]] = []

    def bind(self, loader: ArchiveDetailLoader) -> None:
        loader.loaded.connect(self._on_loaded)
        loader.empty.connect(self.empty.append)
        loader.failed.connect(lambda tok, msg: self.failed.append((tok, msg)))

    @Slot(object)
    def _on_loaded(self, payload: object) -> None:
        if isinstance(payload, ArchiveDetailResult):
            self.loaded.append(payload)


def _t0() -> UTCDateTime:
    return UTCDateTime("2026-06-01T00:00:00")


def test_loads_three_components_off_thread(qtbot, tmp_path) -> None:
    root = tmp_path / "sds"
    t0 = _t0()
    for comp in ("Z", "N", "E"):
        _write_trace(root, comp, t0, npts=int(_FS * 60))

    loader = ArchiveDetailLoader(None)
    col = _Collector()
    col.bind(loader)
    try:
        loader.request(
            _DEVICE,
            _nslc("Z"),
            _components(),
            float(t0 + 10.0),
            float(t0 + 30.0),
            str(root),
        )
        qtbot.waitUntil(lambda: bool(col.loaded) or bool(col.failed), timeout=10_000)
        assert not col.failed, col.failed
        result = col.loaded[-1]
        assert [t.comp for t in result.traces] == ["Z", "N", "E"]  # Z first
        assert result.trigger_comp == "Z"
        for t in result.traces:
            assert t.fs == _FS
            assert t.x[0] >= float(t0 + 10.0) - 1.0
            assert np.isfinite(t.y).any()
        # The worker lives on the loader's dedicated thread, not the test's.
        assert loader._worker.thread() is loader._thread
        assert loader._thread is not QThread.currentThread()
    finally:
        loader.shutdown()


def test_gap_renders_as_nan_break(qtbot, tmp_path) -> None:
    root = tmp_path / "sds"
    t0 = _t0()
    _write_gapped_trace(root, "Z", t0, npts=int(_FS * 60), gap_at=int(_FS * 20))
    loader = ArchiveDetailLoader(None)
    col = _Collector()
    col.bind(loader)
    try:
        loader.request(
            _DEVICE, _nslc("Z"), {"Z": _nslc("Z")}, float(t0), float(t0 + 60.0), str(root)
        )
        qtbot.waitUntil(lambda: bool(col.loaded) or bool(col.failed), timeout=10_000)
        assert not col.failed, col.failed
        z = col.loaded[-1].traces[0]
        # There is at least one NaN (the gap) and at least one finite sample.
        assert np.isnan(z.y).any()
        assert np.isfinite(z.y).any()
    finally:
        loader.shutdown()


def test_missing_components_returns_present_only(qtbot, tmp_path) -> None:
    root = tmp_path / "sds"
    t0 = _t0()
    _write_trace(root, "Z", t0, npts=int(_FS * 30))  # only Z archived
    loader = ArchiveDetailLoader(None)
    col = _Collector()
    col.bind(loader)
    try:
        loader.request(_DEVICE, _nslc("Z"), _components(), float(t0), float(t0 + 20.0), str(root))
        qtbot.waitUntil(lambda: bool(col.loaded) or bool(col.empty), timeout=10_000)
        assert col.loaded, "expected Z to load even though N/E are absent"
        assert [t.comp for t in col.loaded[-1].traces] == ["Z"]
    finally:
        loader.shutdown()


def test_no_data_emits_empty(qtbot, tmp_path) -> None:
    root = tmp_path / "sds"  # nothing written
    t0 = _t0()
    loader = ArchiveDetailLoader(None)
    col = _Collector()
    col.bind(loader)
    try:
        loader.request(_DEVICE, _nslc("Z"), _components(), float(t0), float(t0 + 20.0), str(root))
        qtbot.waitUntil(lambda: bool(col.empty) or bool(col.loaded), timeout=10_000)
        assert col.empty and not col.loaded
    finally:
        loader.shutdown()


def test_corrupt_path_emits_failed_without_crashing_thread(qtbot, tmp_path, monkeypatch) -> None:
    root = tmp_path / "sds"
    t0 = _t0()
    _write_trace(root, "Z", t0, npts=int(_FS * 30))

    calls = {"n": 0}
    real = archive_reader_mod.ArchiveReader.read_window

    def _boom(self, *a, **k):  # type: ignore[no-untyped-def]
        calls["n"] += 1
        if calls["n"] == 1:
            raise RuntimeError("synthetic archive corruption")
        return real(self, *a, **k)

    monkeypatch.setattr(archive_reader_mod.ArchiveReader, "read_window", _boom)

    loader = ArchiveDetailLoader(None)
    col = _Collector()
    col.bind(loader)
    try:
        loader.request(
            _DEVICE, _nslc("Z"), {"Z": _nslc("Z")}, float(t0), float(t0 + 20.0), str(root)
        )
        qtbot.waitUntil(lambda: bool(col.failed), timeout=10_000)
        assert col.failed
        # The thread survived: a second request still works.
        loader.request(
            _DEVICE, _nslc("Z"), {"Z": _nslc("Z")}, float(t0), float(t0 + 20.0), str(root)
        )
        qtbot.waitUntil(lambda: bool(col.loaded), timeout=10_000)
        assert col.loaded
    finally:
        loader.shutdown()


def test_latest_wins_supersedes_stale_load(qtbot, tmp_path) -> None:
    root = tmp_path / "sds"
    t0 = _t0()
    for comp in ("Z", "N", "E"):
        _write_trace(root, comp, t0, npts=int(_FS * 60))
    loader = ArchiveDetailLoader(None)
    col = _Collector()
    col.bind(loader)
    try:
        tok1 = loader.request(
            _DEVICE, _nslc("Z"), _components(), float(t0), float(t0 + 30.0), str(root)
        )
        tok2 = loader.request(
            _DEVICE, _nslc("Z"), _components(), float(t0 + 5.0), float(t0 + 35.0), str(root)
        )
        assert tok2 > tok1
        qtbot.waitUntil(lambda: bool(col.loaded), timeout=10_000)
        qtbot.wait(200)  # allow any stale result to (not) arrive
        # Only the newest token's result is delivered/kept.
        assert all(r.token == tok2 for r in col.loaded)
    finally:
        loader.shutdown()


def test_shutdown_is_bounded(qtbot, tmp_path) -> None:
    root = tmp_path / "sds"
    _write_trace(root, "Z", _t0(), npts=int(_FS * 10))
    loader = ArchiveDetailLoader(None)
    loader.request(
        _DEVICE, _nslc("Z"), {"Z": _nslc("Z")}, float(_t0()), float(_t0() + 5.0), str(root)
    )
    started = time.monotonic()
    loader.shutdown()
    assert time.monotonic() - started < 5.0


# ---------------------------------------------------------------------------
# THE rule-11 invariant: a slow archive load must not stall the live drain.
# ---------------------------------------------------------------------------

_LIVE_NET, _LIVE_STA, _LIVE_LOC = "XX", "DRAIN", "00"
_LIVE_DEVICE = "draingen"
_LIVE_CHANS = ("HHZ", "HHN", "HHE")


def _live_cfg(archive_dir: Path) -> RootConfig:
    return RootConfig(
        app=AppConfig(archive_root=str(archive_dir)),
        ui=UiConfig(refresh_hz=20, default_window_seconds=10),
        devices=[
            DeviceConfig(
                name=_LIVE_DEVICE,
                host="192.0.2.1",
                port=18000,
                reconnect=ReconnectConfig(
                    initial_delay_s=3600.0, max_delay_s=3600.0, connect_timeout_s=0.5
                ),
                selectors=[
                    StreamSelectorConfig(
                        network=_LIVE_NET, station=_LIVE_STA, location=_LIVE_LOC, channel="HH?"
                    )
                ],
                dsp_chain=[HighpassStage(type="highpass", freq=1.0)],
            )
        ],
    )


def _live_trace(cha: str, t0: UTCDateTime, n: int, rng: np.random.Generator) -> Trace:
    tr = Trace(data=(rng.standard_normal(n) * 1000.0).astype(np.int32))
    tr.stats.network, tr.stats.station = _LIVE_NET, _LIVE_STA
    tr.stats.location, tr.stats.channel = _LIVE_LOC, cha
    tr.stats.sampling_rate = _FS
    tr.stats.starttime = t0
    return tr


class _LiveFeeder(QObject):
    packet = Signal(str, object)
    finished = Signal(int)

    def __init__(self, spp: int, n_packets: int) -> None:
        super().__init__()
        self._spp, self._n = spp, n_packets
        self._stop = False

    @Slot()
    def run(self) -> None:
        rng = np.random.default_rng(123)
        t0 = UTCDateTime(0)
        dt = self._spp / _FS
        total = 0
        for _ in range(self._n):
            if self._stop:
                break
            for cha in _LIVE_CHANS:
                self.packet.emit(_LIVE_DEVICE, _live_trace(cha, t0, self._spp, rng))
                total += self._spp
            t0 = t0 + dt
            QThread.msleep(max(1, int(dt * 1000)))
        self.finished.emit(total)

    def stop(self) -> None:
        self._stop = True


def test_slow_archive_load_does_not_stall_live_drain(qtbot, tmp_path, monkeypatch) -> None:
    """A deliberately-slow archive load must not stall the live drain.

    This is the invariant the reverted Archive Replay got wrong: it ran the
    read on the GUI thread, pegging it. Here the read runs on the loader's
    worker thread, so while a 0.8 s read is in flight the engine's live drain
    keeps advancing (``_latest_raw_endtime`` moves forward) and the DSP path
    drops nothing. We assert the drain ADVANCES across the load — not merely
    that buffers are unshared.
    """
    # A real archived window the slow read returns (single component).
    arch_root = tmp_path / "sds"
    a0 = UTCDateTime("2026-06-01T00:00:00")
    # Seed under the SAME device the loader request uses below
    # (_LIVE_DEVICE), so the device-namespaced reader finds the files.
    _write_trace(arch_root, "Z", a0, npts=int(_FS * 30), device=_LIVE_DEVICE)

    real_read = archive_reader_mod.ArchiveReader.read_window

    def _slow_read(self, *a, **k):  # type: ignore[no-untyped-def]
        time.sleep(0.8)  # sleep releases the GIL — the GUI loop keeps draining
        return real_read(self, *a, **k)

    monkeypatch.setattr(archive_reader_mod.ArchiveReader, "read_window", _slow_read)

    engine = StreamingEngine(_live_cfg(tmp_path / "live"))
    processed = {"n": 0, "dropped": 0, "fed": 0, "done": False}
    engine.processedTraceReady.connect(
        lambda _d, _n, s: processed.__setitem__("n", processed["n"] + len(s)),
        type=Qt.ConnectionType.DirectConnection,
    )
    engine.chainDropped.connect(
        lambda _d, _n, c: processed.__setitem__("dropped", processed["dropped"] + int(c)),
        type=Qt.ConnectionType.DirectConnection,
    )
    engine.start()

    loader = ArchiveDetailLoader(None)
    col = _Collector()
    col.bind(loader)

    feeder = _LiveFeeder(spp=20, n_packets=25)  # ~5 s of feed (overlaps the 0.8 s load)
    feeder.finished.connect(lambda total: processed.__setitem__("fed", total))
    feeder.finished.connect(lambda _t: processed.__setitem__("done", True))
    fthread = QThread()
    feeder.moveToThread(fthread)
    feeder.packet.connect(engine._on_packet, type=Qt.ConnectionType.QueuedConnection)
    fthread.started.connect(feeder.run)
    fthread.start()
    try:
        # Wait until the live drain has populated some endtimes.
        qtbot.waitUntil(lambda: bool(engine._latest_raw_endtime), timeout=10_000)

        def _max_endtime() -> float:
            ends = list(engine._latest_raw_endtime.values())
            return max(float(e) for e in ends) if ends else 0.0

        t_before = _max_endtime()
        # Kick off the slow archive load mid-feed.
        loader.request(
            _LIVE_DEVICE, _nslc("Z"), {"Z": _nslc("Z")}, float(a0), float(a0 + 20.0), str(arch_root)
        )
        qtbot.waitUntil(lambda: bool(col.loaded) or bool(col.failed), timeout=10_000)
        assert not col.failed, col.failed
        t_after = _max_endtime()

        # THE invariant: the live drain advanced while the slow load ran.
        assert t_after > t_before, (
            f"live drain did not advance during the archive load "
            f"({t_before} -> {t_after}) — the load stalled the GUI thread"
        )

        qtbot.waitUntil(lambda: processed["done"], timeout=15_000)
        deadline = time.monotonic() + 10.0
        while time.monotonic() < deadline and processed["n"] < processed["fed"]:
            qtbot.wait(50)
        assert processed["fed"] > 0
        assert processed["dropped"] == 0, "DSP dropped packets during the archive load"
        assert processed["n"] == processed["fed"], (
            f"DSP saw {processed['n']}/{processed['fed']} samples"
        )
    finally:
        feeder.stop()
        fthread.quit()
        fthread.wait(2000)
        loader.shutdown()
        engine.stop()
